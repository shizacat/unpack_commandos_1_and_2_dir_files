...
class dirPack(object):
	...
	def _read2(self, fin, start_seek=None):
		...
		while True:
			name, t, s, offset = struct.unpack(self.fmt, fin.read(struct.calcsize(self.fmt)))
	...
	def pack(self, d):
		...
		headerLen = (self.countItemHeader(self.current_dir) + 1) * struct.calcsize(self.fmt)
		headerTree	= []	# Дерево заголовка

		with open(self.filename, "wb") as fout:
			self.fout = fout

			self.sk_file_offset = headerLen		# смещение на место куда надо писать следующий файл
			...
			fout.write( struct.pack(self.fmt, self._getFmtStrName('DIRECTOR.FIN'), self.t_edir, 0, 0xFF))

			self._makeStructDir(root_dir, offset_next)

	def countItemHeader(self, d):
		cListDir = 0	# Количество спискок (директорий у них есть начало и конец)
	...
	def _makeStructDir(self, root_dir, offset):
		'''
			Формирует структуру для указанного каталога и возвращает смещение куда она была записана
		...
		for ditem in dirs:
			...
			listStruct.append( struct.pack(self.fmt, name, self.t_dir, 0, offset_next) )
			offset_next = self._makeStructDir( os.path.join(root_dir, ditem), offset_next)

		for fitem in files:
			name = self._getFmtStrName( os.path.basename( fitem))

			size = self._makeStructFile( os.path.join(root_dir, fitem), self.sk_file_offset)
			p_of = self.sk_file_offset
			self.sk_file_offset = self.sk_file_offset + size

			listStruct.append( struct.pack(self.fmt, name, self.t_file, size, p_of) )

		listStruct.append( struct.pack(self.fmt, self._getFmtStrName('DIRECTOR.FIN'), self.t_edir, 0, 0xFF) )
...
if __name__ == '__main__':
	...
	parser_p = subparsers.add_parser('p', help='Запаковать указанный каталог в файл')

	parser_u.add_argument('-f', action='store', required=True, help='Название файла, который будет распакован в текущию директорию')

	parser_p.add_argument('-d', action='store', required=True, help='Название каталога, который нужно запаковать')
	parser_p.add_argument('-f', action='store', required=True, help='Имя файла, в который будет записан архив')
